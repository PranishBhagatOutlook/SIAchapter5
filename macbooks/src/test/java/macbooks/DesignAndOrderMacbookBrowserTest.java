...
public class DesignAndOrderMacbookBrowserTest {
  ...
  @Test
  public void testDesignAMacbookPage_HappyPath() throws Exception {
    ...
    buildAndSubmitAMacbook("Basic Macbook", "FLTO", "GRBF", "CHED", "TMTO", "SLSA");
    clickBuildAnotherMacbook();
    buildAndSubmitAMacbook("Another Macbook", "COTO", "CARN", "JACK", "LETC", "SRCR");
  ...
  @Test
  public void testDesignAMacbookPage_EmptyOrderInfo() throws Exception {
    browser.get(homePageUrl());
    clickDesignAMacbook();
    assertLandedOnLoginPage();
  ...
  @Test
  public void testDesignAMacbookPage_InvalidOrderInfo() throws Exception {
    browser.get(homePageUrl());
    clickDesignAMacbook();
    assertLandedOnLoginPage();
  ...
  //
  private void buildAndSubmitAMacbook(String name, String... ingredients) {
    assertDesignPageElements();
    ...
    browser.findElementByCssSelector("form#macbookForm").submit();
  }
  ...
  private void clickDesignAMacbook() {
    assertEquals(homePageUrl(), browser.getCurrentUrl());
    browser.findElementByCssSelector("a[id='design']").click();
  }

  private void clickBuildAnotherMacbook() {
    assertTrue(browser.getCurrentUrl().startsWith(orderDetailsPageUrl()));
